...
!function () {
  ...
  if (true) !(__WEBPACK_AMD_DEFINE_FACTORY__ = (topojson),
				...
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));else if ((typeof module === "undefined" ? "undefined" : _typeof(module)) === "object" && module.exports) module.exports = topojson;else this.topojson = topojson;
}();
...
function drawNeighborhoodContainers(arrNeighborhood) {
    ...
    for (var i = 0; i < arrNeighborhood.length; i++) {
        ...
        container1.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("class", "empty-state").append("g").style("transform", "translate(" + textcenter + "px" + ", 4.5em)").append("text").html("no data").style("opacity", ".5");

        container2.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("class", "empty-state").append("g").style("transform", "translate(" + textcenter + "px" + ", 4.5em)").append("text").html("no data").style("opacity", ".5");

        container3.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("class", "empty-state").append("g").style("transform", "translate(" + textcenter + "px" + ", 4.5em)").append("text").html("no data").style("opacity", ".5");

        container4.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("class", "empty-state").append("g").style("transform", "translate(" + textcenter + "px" + ", 4.5em)").append("text").html("no data").style("opacity", ".5");
    }
    drawBuyouts();
    drawEvictions();
...
function drawGraphsByNeighborhoods(data, id, yScaleMax) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var path = svg.selectAll("path").data([data[i].values]);

        path.enter().append("path").attr("class", "line").attr("d", valueline).attr("clip-path", "url(#clip)");

        path.exit().remove();

        // Add the X Axis
        svg.append("g").attr("transform", "translate(0," + height + ")").attr("class", "axis").call(d3.axisBottom(x).tickFormat(d3.format("d")).tickValues(x.domain()));

        // Add the Y Axis
        svg.append("g").attr("class", "axis").call(d3.axisLeft(y).tickValues(y.domain()));
    }
}
...
function drawEvictions() {
    ...
    d3.csv('/../data/Count_of_Eviction_Notices_By_Analysis_Neighborhood_and_Year.csv', function (data) {
        var nested_data = d3.nest().key(function (d) {
            if (d.Neighborhoods_Analysis_Boundaries != "" && d.Neighborhoods_Analysis_Boundaries != "Lakeshore" && d.Neighborhoods_Analysis_Boundaries != "Oceanview/Merced/Ingleside") {
                return d.Neighborhoods_Analysis_Boundaries;
            ...
            var sum = d3.sum(leaves, function (d) {
                return d.Count_of_Eviction_Notices;
            });
...
function drawWater(svg, centroid) {
    ...
    d3.json("/../data/water.json", function (error, data) {
        ...
        proj = d3.geoMercator().center(center).scale(scale).translate(offset);

        path = d3.geoPath(proj);

        var paths = svg.selectAll(".water").data(featureCollection.features);

        paths.enter().append("path").attr("d", path).attr("class", "water").style("stroke", "#C2EDEB").style("stroke-width", "3px").attr("fill", "#C2EDEB");
    });
}
...
function drawMap() {
    ...
    d3.json("/../data/map.json", function (error, data) {
        ...
        var svg1 = d3.select("#map").append("svg").attr("width", "100%").attr("height", "100%").attr("class", "map-svg").style("background", "#C2EDEB");
        ...
        var paths = svg.selectAll("path").data(featureCollection.features);

        paths.enter().append("path").attr("d", path).attr("class", function (d, i) {
            return i;
        }).style("stroke", "rgba(149, 152, 154, .25)").style("stroke-width", "1px").attr("fill", "#535353");
...
function drawNeighborhoods(svg, proj) {
    ...
    d3.json("/../data/neighborhoods.json", function (error, data) {
        ...
        var nested_data = d3.nest().key(function (d) {
            return d[10];
        }).sortKeys(d3.ascending).entries(data.data);

        var poly = svg.selectAll("polygon").data(nested_data);

        poly.enter().append("polygon").attr("class", "lol").attr("points", function (d) {
            ...
            return pointsString;
        }).attr("stroke", "#ffa884").attr("stroke-width", "1px").attr("stroke-opacity", ".5").attr("fill", "transparent").on("mouseover", function (d) {
            d3.select(this).attr("fill", "#ffa884").attr("fill-opacity", ".25");
            ...
            var tooltip = d3.select(".map-svg").append("svg").attr("class", "tool-tip").attr("x", center[1]).attr("y", center[0]);

            tooltip.append("foreignObject").attr("width", "150").attr("height", 60).append("xhtml:div").style("font", "14px 'Helvetica Neue'").style("background", "rgba(0, 0, 0, 0.7)").style("padding", "0.75em 0.5em").style("width", "auto").style("text-align", "center").style("font-weight", 900).html(function () {
                return d.key;
            });
...
function drawBusinesses(svg, year, proj) {
    ...
    d3.csv('/../data/Registered_Business_Locations_-_San_Francisco.csv', function (data) {
        ...
        points.enter().append("circle").attr("r", 1).attr("class", function (d) {
            var className = d.Business_Location;
            return className;
        }).attr("cx", function (d) {
            if (d.Business_Location != "" && d.Business_Location.indexOf('(') != -1 && d.Business_Location.indexOf('SAN FRANCISCO, CA') != -1) {
        ...
        }).attr("fill", "orange").style("opacity", 1);
...
function drawPetitions(svg, year, proj) {
    ...
    d3.csv('/../data/Petitions_to_the_Rent_Board.csv', function (data) {
        ...
        points.enter().append("circle").attr("r", 1).attr("class", function (d) {
            ...
            return coord;
        }).attr("fill", "yellow").style("opacity", 1);
...
function drawBuyOuts(svg, year, proj) {
    ...
    d3.csv('/../data/Buyout_agreements.csv', function (data) {
        ...
        points.enter().append("circle").attr("r", 1).attr("class", function (d) {
            ...
            return coord;
        }).attr("fill", "turquoise").style("opacity", 1);
...
function mapDrawEvictions(svg, year, proj) {
    ...
    d3.csv('/../data/Eviction_Notices2.csv', function (data) {
        ...
        points.enter().append("circle").attr("r", 4).attr("class", function (d) {
            ...
            return coord;
        }).attr("fill", "red").attr("stroke", "red").style("fill-opacity", .1).style("stroke-opacity", .7);
